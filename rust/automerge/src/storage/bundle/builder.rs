...
impl<'a> BundleOpWriter<'a> {
    fn add(&mut self, op: OpBuilder<'a>, _index: usize, mapper: &mut ActorMapper<'a>) {
        ...
        self.i_pos.push((op.id.actor(), op.id.counter(), doc_pos));
    }
    ...
    fn finish(
        ...
        // Capture doc-order counters before sorting `i_pos`.
        ...
        self.i_pos.sort_unstable();
        let id_ctr_inverse =
            hexane::v1::DeltaEncoder::<u32>::encode_to(data, self.i_pos.iter().map(|p| p.2));
